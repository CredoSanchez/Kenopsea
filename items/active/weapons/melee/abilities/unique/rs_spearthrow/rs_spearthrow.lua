...
RSSpearThrow = WeaponAbility:new()

function RSSpearThrow:init()
  self:reset(true)
...
function RSSpearThrow:update(dt, fireMode, shiftHeld)
  ...
  elseif self.thrownProjectile and self.fireMode == (self.activatingFireMode or self.abilitySlot) then
	world.sendEntityMessage(self.thrownProjectile, "kill")
  end
end

function RSSpearThrow:windup()
...
function RSSpearThrow:aiming()
  if self.windupTimer == 0 then
	self.weapon:setStance(self.stances.aiming)
  end
  
  --Optionally play sounds
...
function RSSpearThrow:cooldown()
  self.weapon:updateAim()
...
function RSSpearThrow:idealAimVector()
...
function RSSpearThrow:firePosition()
  return vec2.add(mcontroller.position(), activeItem.handPosition(self.fireOffset))
end

function RSSpearThrow:reset(forceCooldown)
